...
> 当前为 **0.9.1 预览版**。主要功能已实现，性能与长期稳定性仍在持续验证。发布文件暂未签名。
...
内置系统快捷包括计算器、文件资源管理器、任务管理器、命令提示符、控制面板、卸载或更改程序、环境变量和设备管理器；无需固定即可按中文名称、英文、拼音或缩写搜索，例如 `calculator`、`cmd`、`rwglq`、`hjbl`。点击主页面内其他区域时搜索框保持焦点；有输入时按 `Esc` 清空并回到主页，再按一次隐藏窗口。
