...
> 当前为 **0.9.3 预览版**。主要功能已实现，性能与长期稳定性仍在持续验证。发布文件暂未签名。
...
- **桌面快捷方式**：扫描 Windows 实际的用户桌面（包含 OneDrive 重定向）和公共桌面，纳入指向现存文件的 `.lnk`；跳过空目标、失效目标和 `.url` 网络/协议快捷方式。同一应用在桌面和开始菜单都有入口时，优先保留桌面名称与快捷方式，其他名称仍可搜索。
...
添加应用与首页固定独立：新添加的应用不会自动固定，搜索结果旁的星标按钮 可将应用固定到首页，取消固定后仍可搜索。自定义应用和固定记录统一保存在 `application-library.json` 中；旧版 `quick-launch.json` 中的手动应用会自动迁移，原文件保留为备份。

搜索优先考虑固定项、启动历史、桌面/开始菜单入口及常见应用，并降低更新服务等辅助程序的排序；精确名称匹配仍优先。常用排序只使用 Velo 的本地启动记录，不读取其他启动器的历史或固定列表。

系统快捷包含 40 个入口定义，会在启动和刷新索引时根据 Windows 版本、本机组件及设置页策略筛选。除计算器、环境变量等常用工具外，还包括系统配置、系统信息、计算机管理、事件查看器、磁盘清理，以及显示、颜色、激活、恢复、更新等设置页。首页默认展示 12 项，可“展开全部”，支持固定和按使用记录排序。无需固定即可按中文、英文、拼音或缩写搜索，例如 `xt`、`msconfig`、`rwglq`、`hjbl`。入口只打开 Windows 页面或工具，具体操作由用户在其中完成；详见[实现与兼容性说明](docs/system-shortcuts.md)。点击主页面内其他区域时搜索框保持焦点；有输入时按 `Esc` 清空并回到主页，再按一次隐藏窗口。
...
是。正常模式不显示任务栏按钮，失焦或在输入为空时按 `Esc` 只会隐藏窗口；使用托盘菜单或窗口底部“退出”结束程序。
...
本项目的主要图标库为 [Microsoft Fluent UI System Icons](https://github.com/microsoft/fluentui-system-icons)（MIT）。系统快捷入口及搜索、设置、刷新、固定等操作统一使用官方 SVG，保留第三方应用的原生图标。图标以本地子集打包，无需联网；后续新增图标请遵循 [图标选型与接入规范](frontend/src/assets/fluent/README.md)。完整许可可在应用“设置 → 关于”中查看。
